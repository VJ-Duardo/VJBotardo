...
var emoteOrigins;
loadOrigins().then(function (list) {
  emoteOrigins = list;
});
...
var modes = {
  global: ["ffzGlobal", "bttvGlobal", "twitchGlobal", "seventvGlobal"],
  channel: ["ffzChannel", "bttvChannel", "twitchChannel", "seventvChannel"],
};
modes.all = [...modes.global, ...modes.channel];

module.exports = {
  guessTheEmote: function (channelObj, sayFunc, user, command) {
    if (
      command[0] + command[1] === channelObj.prefix + "guessstop" &&
    ...
    if (!getGameState(channelObj.name)) {
      ...
      if (newGame === -1) {
        sayFunc(
          channelObj.name,
          `/me Invalid mode! Has to be "global", "channel", "all" or "origin" (e.g. ${channelObj.prefix}guess all 5)`
        );
        return;
      } else if (newGame === -2) {
      ...
      if (getGameSolution(channelObj.name).toLowerCase() === command[0].toLowerCase()) {
        let winString = `/me ${user["display-name"]} guessed it right! It's ${getGameSolution(
          channelObj.name
        )} (+${reward}USh)`;
...
async function startGame(channelObj, gameObj, sayFunc) {
  ...
  gameObj.firstHint = setTimeout(function () {
    giveFirstHint(channelObj, gameObj, sayFunc);
  }, firstHintTime);
  gameObj.secondHint = setTimeout(function () {
    giveSecondHint(channelObj, gameObj, sayFunc);
  }, secondHintTime);
  let loseString = `/me It was ${gameObj.solution.name} . Disappointing performance :Z`;
  gameObj.resolve = setTimeout(function () {
    resolveRound(channelObj, gameObj, sayFunc, loseString);
  }, resolveTime);

  if (gameObj.mode !== "origin") {
    ascii
      .ascii("ascii", [gameObj.solution.url], false, ["-tr", "255"], null, null)
...
function giveSecondHint(channelObj, gameObj, sayFunc) {
  if (gameObj.mode === "origin") {
    ...
    ascii
      .ascii("ascii", [gameObj.solution.url], false, ["-r", randomRoation, "-d"], null, null)
      .then(async (brailleString) => {
        if (brailleString === -1) {
          return;
        } else {
...
async function resolveRound(channelObj, gameObj, sayFunc, endString) {
  ...
  if (games[channelObj.name].rounds === 0) {
    ...
    setTimeout(function () {
      startGame(channelObj, games[channelObj.name], sayFunc);
    }, timeBetweenRounds);
  }
}
  ...
  switch (gameObj.originHints.length) {
    case 0:
      if (gameObj.originLastHint) {
        ...
        return `____${gameObj.solution.name.substring(
          Math.floor(gameObj.solution.name.length * hidePercent),
          gameObj.solution.name.length
        )}`;
      }
    default:
