...
function parseData(country, dateStart, dateEnd, sayFunc, channelObj) {
  ...
  for (let i = 0; i < csvData.length; i++) {
    if (regex.test(csvData[i])) {
      let numericalData = csvData[i]
        .split(",")
        .slice(4 + diffDaysStart, diffDaysEnd + 4)
        .map(function (x) {
          return parseInt(x);
        });
...
function createHistogram(data, bins, height, sayFunc, channelObj) {
  ...
  for (let i = 0; i < bins; i++) {
    ...
    histogramData.push(
      bin.reduce(function (accumulator, currentValue) {
...
function loadData() {
  fetch(
    ...
    .then((text) => {
      if (text !== "" && text) csvData = text.toLowerCase().split(/\r?\n/);
    })
    .catch(function (e) {
      console.log(e);
    });
...
async function corona(channelObj, sayFunc, userInput, gifSpam) {
  ...
  ["-f", "-t"].forEach((opt, i) => {
    ...
    if (optIndex !== -1 && optIndex + 1 < userInput.length) {
      ...
      if (
        ...
        newDate.getTime() >= dateStart &&
        newDate.getTime() <= dateEnd.getTime()
      )
        dateVars[i] = newDate;
    }
  });
  if (dateVars[0] < dateVars[1]) (dateStart = dateVars[0]), (dateEnd = dateVars[1]);
  ...
  ["-h", "-w"].forEach((opt, i) => {
    ...
    if (
      optIndex !== -1 &&
      optIndex + 1 < userInput.length &&
      !isNaN(parseInt(userInput[optIndex + 1]))
    ) {
  ...
  if ((sizeVars[1] / 2) * (sizeVars[0] / 4) <= maxCharacters) {
    ...
    sayFunc(
      channelObj.name,
      `/me You reached the character limit ${maxCharacters}. Adjust your height and width.`
    );
    return;
  }
  ...
  inputCountry = Array.from(
    inputCountry === -1 ? userInput.join(" ") : userInput.slice(0, inputCountry).join(" ")
  )
    .map(emojiToLetter)
    .join("")
    .toLowerCase();
  ...
  if (gifMode) {
    ...
    for (let i = 0; i < frames; i++) {
      ...
      if (
        coronaGenAscii(country, dateStart, movingDate, width, height, sayFunc, channelObj) === -1
      ) {
        break;
      }
