...
async function getUrlByInput(channelObj, input) {
  const regexResult = /https:\/\/cdn\.7tv\.app\/emote\/[a-z0-9]+\/\dx\.(?<ending>[a-z]+)/.exec(
    input
  );
  if (regexResult && regexResult.groups["ending"] === "webp") {
    return await emotes.getSeventvEmoteUrl(input);
  }
...
function createOptionsObj(optionsInput) {
  ...
  if (optionsInput.length === 0) return optionsObj;
  ...
  ["-d", "-i", "-g", "-b", "-e"].forEach(function (opt) {
    if (optionsInput.includes(opt)) optionsObj[givenOptions[opt].descr] = null;
  });
  ...
  if (height < lineHeight) {
    ...
    textObj["textLines"] =
      typeof text !== "undefined"
        ? [
            text
              .slice(0, Math.floor(textObj["width"] * charsPerWidth))
              .split("")
              .join(" ") + " ",
          ]
        : [" "];
...
async function printOversizedAscii(channelObj, sayFunc, gifSpam, brailleString, extra = "") {
  if (gifSpam) {
    ...
    sayFunc(
      channelObj.name,
      `${extra} ${brailleString.substring(0, maxCharactersPerSend).match(/(.+ )+/g)[0]}`
    );
  }
}
...
async function printAscii(channelObj, sayFunc, mode, userInput, gifSpam) {
  if (userInput.length < asciiModes[mode].params) {
    sayFunc(
      channelObj.name,
      "/me Parameter(s) are missing :Z Available extra options: -w, -h, -r, -d, -i, -tr, -t, -g, -b, -e Check commands list for more info."
    );
    return;
  }
  ...
  if (brailleString !== -1) {
    ...
    sayFunc(
      channelObj.name,
      `/me Cant find emote in this channel or invalid link :Z If you added a new emote, do ${channelObj.prefix}reload`
    );
    return -1;
  }
  return 1;
...
async function ascii(mode, urls, gifSpam, asciiOptions, channelObj, sayFunc) {
  ...
  if (
    ...
    options["height"] > maxHeightOverall ||
    options["width"] < 1 ||
    options["height"] < 1
  ) {
  ...
  let brailleText =
    textObject !== null && textObject["textLines"].length > 0
      ? generateTextAscii(textObject, !options.hasOwnProperty("background"))
      : "";
  let treshold = options.hasOwnProperty("treshold") ? options["treshold"] : -1;
  let brailleResult = `${braille.iterateOverPixels(
  ...
  brailleResult = options.hasOwnProperty("empty")
    ? brailleResult.replace(/[⠄]/g, "⠀")
    : brailleResult;
...
async function plainAscii(width, height, context, srcLeft, srcRight) {
  return await addPicsToContext(context, [{ url: srcLeft, x: 0, y: 0 }], {
    width: width,
    height: height,
  });
}
...
async function mix(width, height, context, srcTop, srcBottom) {
  ...
  context.save();
  context.clip();
  context = await addPicsToContext(context, [{ url: srcTop, x: 0, y: 0 }], {
    width: width,
    height: height,
  });
  ...
  context.closePath();
  context.clip();
  return await addPicsToContext(context, [{ url: srcBottom, x: 0, y: 0 }], {
    width: width,
    height: height,
  });
}
...
async function mirrorFunctions(width, height, context, src, clipReg) {
  ...
  context.save();
  context.clip();
  context = await addPicsToContext(context, [{ url: src, x: 0, y: 0 }], {
    width: width,
    height: height,
  });
  if (context === -1) return context;
  ...
  context.closePath();
  context.clip();
  return await addPicsToContext(context, [{ url: src, x: 0, y: 0 }], {
    width: width,
    height: height,
  });
}
...
async function printGifAscii(channelObj, sayFunc, mode, asciiOptions, src, index) {
  ...
  return gifFrames({ url: src[index], frames: "all", outputType: "png", cumulative: cumulativeVal })
    .then(async function (frameData) {
      ...
      for (let i = 0; i < frameData.length; i += frameJump) {
        ...
        let prom = new Promise(function (resolve, reject) {
          let stream = frameData[i]
            .getImage()
            .pipe(fs.createWriteStream("./frames/frame" + i + ".png"));
          stream.on("finish", async function () {
            src[index] = `./frames/frame${i}.png`;
            let status = await printAscii(
              channelObj,
              sayFunc,
              mode,
              src.concat(asciiOptions),
              false
            );
            if (status === -1) reject();
            else resolve();
    ...
    .catch((error) => {
      sayFunc(
        channelObj.name,
        "/me Cant find emote in this channel or invalid link :Z If you added a new emote, do reload"
      );
...
async function gifCheck(src) {
  for (let i = 0; i < src.length; i++) {
    ...
    if (regexResult) {
      switch (regexResult.groups["ending"]) {
        case "gif":
          return i;
        default:
          return -1;
...
async function randomAscii(channelObj, sayFunc, gifSpam, userInput) {
  let keyword =
    typeof userInput[0] !== "undefined" && userInput[0].charAt(0) === "-" ? "" : userInput[0];
  ...
  if (emote === -1) {
    ...
    ascii("ascii", [emote.url], gifSpam, userInput, channelObj, sayFunc).then(
      async (brailleString) => {
        await new Promise((resolve) => setTimeout(resolve, frameDelay));
        if (brailleString === -1) {
