...
const rings = [
  ...
  new Ring(
    ...
    0,
    "BibleThump You missed the board and hit my beautiful wall instead."
  ),
];
...
class Game {
  constructor(channelObj, sayFunc, playerID, playerName) {
    ...
    this.startHandle = setTimeout(function () {
      startGame();
    }, timeToNextRound);
  ...
  async generateRandomPointAscii() {
    ...
    await this.sayFunc(
      ...
      `/me Round ${this.round}/${maxRounds} ${printField(this.context)}`
    );
    await new Promise((resolve) => setTimeout(resolve, postDelay));
    await this.sayFunc(this.channelObj.name, `/me You have ${secondsToInput} seconds!`);
    this.waitForInput.status = true;
    this.waitForInput.handle = setTimeout(function () {
      _this.evaluateRound("0r", "[Out of time]");
    }, secondsToInput * 1000);
  }
  ...
  addPreviousHits(hits) {
    ...
    for (const hit of hits) {
      if (hit.x < 0 || hit.x > pixelWidth || hit.y < 0 || hit.y > pixelHeight) continue;
      this.context.fillText("x", hit.x, hit.y + yTextCorrection);
    }
  }

  async evaluateRound(input, origin) {
    if (!new RegExp(/^(\d+[udlr] )*(\d+[udlr] ?)$/g).test(input)) return;
    ...
    for (let chunk of input.split(" ")) {
      ...
      if (
        optionIndex !== -1 &&
        optionIndex !== 0 &&
        !isNaN(parseInt(characters.slice(0, optionIndex).join("")))
      ) {
    ...
    let distanceFromMiddle = Math.sqrt(
      (pixelWidth / 2 - this.currentPoint.x) ** 2 + (pixelHeight / 2 - this.currentPoint.y) ** 2
    );
    let ring = rings.find(
      (elem) => distanceFromMiddle >= elem.start && distanceFromMiddle <= elem.end
    );
    ...
    await this.sayFunc(
      this.channelObj.name,
      `/me ${origin} ${ring.message}${ring.getPointsString()} Points overall: ${
        this.getPlayerByIndex(this.currentPlayer).points
      }`
    );
    await new Promise((resolve) => setTimeout(resolve, postDelay));
  ...
  updateGameStatus() {
    if (this.round === maxRounds && this.currentPlayer + 1 >= Object.keys(this.players).length) {
      ...
      this.sayFunc(
        this.channelObj.name,
        `/me ${this.getPlayerByIndex(this.currentPlayer).name}, Get ready for the next round...`
      );
      this.nextRoundHandle = setTimeout(function () {
        _this.generateRandomPointAscii();
      }, timeToNextRound);
    }
  }
  ...
  async endGame() {
    if (Object.keys(this.players).length > 0) {
      ...
      this.sayFunc(
        this.channelObj.name,
        `/me Game is over! You got ${player.points} points and earned ${player.points}USh :D`
      );
...
class GameParty extends Game {
  constructor(channelObj, sayFunc, playerID, playerName) {
    ...
    this.waitForJoin = {
      ...
      handle: setTimeout(function () {
        _this.startGame.bind(_this)();
      }, timeToWaitForPlayers),
    };
  ...
  startMessage(player) {
    this.sayFunc(
      this.channelObj.name,
      `/me A new game of darts has been started! Type ${this.channelObj.prefix}join to play! \
  ...
  addPlayer(playerName, playerID) {
    if (Object.keys(this.players).length === maxPlayers || this.players.hasOwnProperty(playerID)) {
      ...
      this.sayFunc(
        this.channelObj.name,
        `/me [${Object.keys(this.players).length}/${maxPlayers}] ${playerName} joined!`
      );
  ...
  startGame() {
    ...
    this.sayFunc(
      this.channelObj.name,
      `/me The Game is starting, ${this.getPlayerByIndex(this.currentPlayer).name}, Get ready...`
    );
  ...
  concede(id) {
    ...
    delete this.players[id];
    if (this.waitForJoin.status && Object.keys(this.players).length > 0) return;
  ...
  async endGame() {
    if (Object.keys(this.players).length >= 1 && !this.waitForJoin.status) {
      let standingsList = Object.keys(this.players).sort(
        (a, b) => this.players[b].points - this.players[a].points
      );
      let winner = this.players[standingsList[0]];
      let reward = parseInt(
        this.players[standingsList[0]].points * (standingsList.length / maxPlayers + 1)
      );
      this.sayFunc(
        this.channelObj.name,
        `/me Game is over! Final standings: ${standingsList
          .map(
            (id, i) =>
              (id = `${i + 1}. \
            ${this.players[id].name}: ${this.players[id].points}`)
          )
          .join(" | ")}. ${winner.name} wins ${reward} USh!`
      );
      await db.addUserPoints(winner.id, winner.name, reward);
      for (const id of standingsList) {
        await db.setHighscoreIfHigh(
          this.players[id].id,
...
module.exports = {
  playDarts: function (channelObj, sayFunc, user, input) {
    if (!games.hasOwnProperty(channelObj.name)) {
      switch (input[1]) {
        case "howtoplay":
          sayFunc(
            channelObj.name,
            "/me A hand holding an arrow will appear in a random location on the board. " +
              "" +
        ...
        case "normal":
          if (input[1] === "normal")
            ...
            games[channelObj.name] = new GameParty(
              channelObj,
              sayFunc,
              user["user-id"],
              user["username"]
        ...
        case undefined:
          ...
          sayFunc(
            channelObj.name,
            "/me Available commands: " +
              p +
              "darts howtoplay - You should read this before playing to know the controls and rules, " +
              p +
              `darts normal - A ${maxRounds} round game for one player only, ` +
              p +
              `darts party - Up to ${maxPlayers} players take turns for ${maxRounds} rounds, with the player with the most points a the end winning!`
          );
      }
      return;
    }
    ...
    if (
      input[0] === channelObj.prefix + "concede" &&
      gameObj.players.hasOwnProperty(user["user-id"])
    ) {
    ...
    if (
      gameObj instanceof GameParty &&
      gameObj.waitForJoin.status &&
      input[0] === channelObj.prefix + "join"
    ) {
    ...
    if (
      ...
      user["user-id"] === gameObj.getPlayerByIndex(gameObj.currentPlayer).id
    ) {
