...
module.exports = {
      ...
      db.get(sql, [name], function (err, row) {
        ...
        if (typeof row === "undefined") {
          ...
          row.url =
            emoteParts.length < 1
              ? row.url
              : row.url.replace(/(\d+(?=\/\d\.0))/, "$1_" + emoteParts[1]);
          resolve(row);
        }
      });
    });
  ...
  getHighScore: function (id, type) {
    return new Promise(function (resolve) {
      switch (type) {
        case "snake":
          type = "snake_highscore";
          break;
        case "darts":
          type = "darts_highscore";
          break;
      }
  ...
  setHighscoreIfHigh: async function (id, name, score, type) {
    ...
    if (userStatus) {
      ...
      if (type === "snake") insertNewUser(id, name, 0, score, 0);
      else insertNewUser(id, name, 0, 0, score);
    }
  },
    ...
    return new Promise(function (resolve) {
      ...
      db.all(sql, [top], (err, row) => {
        ...
        resolve(
          row
            .map((user, index) =>
              row.findIndex((obj) => obj[type] === user[type]) === -1
                ? index + 1
                : row.findIndex((obj) => obj[type] === user[type]) +
                  1 +
                  ". " +
                  user.username +
                  " - " +
  ...
  insertNewCommand: function (name, cooldown, minCooldown, maxCooldown, devOnly, changeable) {
    return new Promise(function (resolve) {
      let sql =
        "INSERT INTO COMMAND(command_name, cooldown, min_cooldown, max_cooldown, dev_only, changeable) VALUES(?, ?, ?, ?, ?, ?)";
    ...
    return new Promise(function (resolve) {
      ...
      switch (option) {
        ...
        case "gifSpam":
          ...
          break;
        case "banphraseAPI":
          column = "banphrase_api";
          break;
        case "allowIfPajbotDown":
          column = "allow_if_pajbot_down";
          break;
        case "emotesString":
          column = "emotes_string";
          break;
        default:
          resolve(-1);
          return;
...
function insertNewUser(id, name, points, snakeHighscore, dartsHighscore) {
  return new Promise(function (resolve) {
    let sql =
      "INSERT INTO USER(id, username, points, snake_highscore, darts_highscore) VALUES (?, ?, ?, ?, ?)";
