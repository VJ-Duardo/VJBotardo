...
function iterateOverPixels(
  ...
  for (i = 0; i < pixelArray.length; i += width * 4) {
    ...
    for (j = 0; j < width; j += 2) {
      line +=
        brailleData.braille_descr_dic[
          getBrailleCode(pixelArray, i + j, width, treshold, setBackground)
        ];
    }
...
function floydSteinberg(pixelArray, width) {
  for (i = 0; i < pixelArray.length; i++) {
    ...
    for (const n of neighbours) {
      if (
        i + n[1] * width + n[0] < pixelArray.length &&
        (i % width) + n[0] >= 0 &&
        (i % width) + n[0] < width
      ) {
        ...
        quantError.forEach(
          (elem, index) =>
            (newColors[index] = Object.values(pixelArray[pixelIndex])[index] + (elem * n[2]) / 16)
        );
...
function getBrailleCode(pixelArray, pos, width, treshold, setBackground) {
  ...
  for (k = 0; k < 2; k++) {
    if (Math.ceil((pos + 1) / width) !== Math.ceil((pos + 1 + k) / width)) continue;
    for (l = 0; l < 4; l++) {
      if (pos + k + width * l < pixelArray.length) {
        if (
          evaluatePixel(
            pixelArray[pos + k + width * l],
            pos + k + width * l,
            treshold,
            setBackground
          )
...
function evaluatePixel(pixel, pos, treshold, setBackground) {
  if (pixel.alpha === 0 && !setBackground) return false;
  else return (pixel.alpha === 0 && setBackground) || pixel.getAvg() > treshold[pos];
}
...
function getAverageColor(pixelArray, width, mask) {
  ...
  maskHeight =
    maskHeight > Math.ceil(pixelArray.length / width)
      ? Math.ceil(pixelArray.length / width)
      : maskHeight;
