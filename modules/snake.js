...
class Game {
  ...
  checkGameOver() {
    return (
      this.snake.isOutOfBounds(fieldWidth, fieldHeight) ||
  ...
  manageApples() {
    ...
    if (freeCell === -1) {
      ...
      this.apples[freeCell] = new Apple(
        freeCell[0],
        freeCell[1],
        elemWidth,
        elemHeight,
  ...
  getFreeCell() {
    ...
    if (freeCells.length < 1) return -1;
    else return freeCells[Math.floor(Math.random() * freeCells.length)];
  }
  ...
  processInput(input) {
    ...
    switch (input) {
      case "a":
        if (this.snake.direction !== "east") this.snake.direction = "west";
        break;
      case "w":
        if (this.snake.direction !== "south") this.snake.direction = "north";
        break;
      case "d":
        if (this.snake.direction !== "west") this.snake.direction = "east";
        break;
      case "s":
        if (this.snake.direction !== "north") this.snake.direction = "south";
        break;
      default:
        return -1;
    }
...
class GameRoyale extends Game {
  ...
  addPlayer(playerName, playerID) {
    if (
      Object.keys(this.players).length === maxRoyalePlayers ||
      ...
      switch (Object.keys(this.players).length) {
        case 2:
          this.sayFunc(
            ...
            `/me [2/${maxRoyalePlayers}] ${playerName}, you are starting from the bottom right!`
          );
        ...
        case 3:
          this.sayFunc(
            this.channelObj.name,
            `/me [3/${maxRoyalePlayers}] ${playerName}, you are starting from the top right!`
          );
        ...
        case 4:
          this.sayFunc(
            this.channelObj.name,
            `/me [4/${maxRoyalePlayers}] ${playerName}, you are starting from the bottom left! Game is starting in a few seconds...`
          );
  ...
  processInput(id, input) {
    ...
    if (player.turnDone || player.out) return;

    switch (input.toLowerCase()) {
      case "a":
        if (player.snake.direction !== "east") player.snake.direction = "west";
        break;
      case "w":
        if (player.snake.direction !== "south") player.snake.direction = "north";
        break;
      case "d":
        if (player.snake.direction !== "west") player.snake.direction = "east";
        break;
      case "s":
        if (player.snake.direction !== "north") player.snake.direction = "south";
        break;
      default:
        return -1;
    }
  ...
  checkGameOver(snake) {
    return (
      typeof []
        .concat(...Object.values(this.players).map((obj) => (obj = obj.snake.body.slice(1))))
        .find((cell) => cell.x === snake.head.x && cell.y === snake.head.y) !== "undefined" ||
  ...
  gameOver() {
    ...
    this.sayFunc(
      this.channelObj.name,
      `/me The game is over! ${Object.values(this.players)
        .map((obj) => (obj = `${obj.place}. ${obj.name}`))
        .sort()
        .join(" | ")}. \
...
class Snake {
  ...
  isOutOfBounds(x, y) {
    return this.head.x >= x || this.head.x < 0 || this.head.y >= y || this.head.y < 0;
  }
}
...
module.exports = {
  playSnake: function (channelObj, sayFunc, user, input) {
    switch (input[1]) {
      case undefined:
        if (input[0] !== channelObj.prefix + "snake") break;
        let p = channelObj.prefix;
        sayFunc(
          channelObj.name,
          `/me Use ${p}snake start to see the available modes, the controls are w a s d. ${p}snake score too see your highscore :)`
        );
        break;
      case "score":
      ...
      case "start":
        if (!games.hasOwnProperty(channelObj.name)) {
          switch (input[2]) {
            case "chat":
              games[channelObj.name] = new Game(
                channelObj,
                sayFunc,
                channelObj.name + "'s chat",
                channelObj.id + String(channelObj.id),
                true
              );
              break;
            case "royale":
              games[channelObj.name] = new GameRoyale(
                channelObj,
                sayFunc,
                user["username"],
                user["user-id"]
              ...
              sayFunc(
                channelObj.name,
                `/me A new round of Snake Royale has started! PogChamp Type ${channelObj.prefix}join to play! \
            ...
            default:
              ...
              sayFunc(
                ...
                "/me Modes: " +
                  p +
                  "snake start normal - normal game for one player, " +
                  p +
                  "snake start chat - normal game where everyone in chat can give input, " +
                  p +
                  `snake start royale - a curve fever inspired game for up to ${maxRoyalePlayers} players.`
              );
              return;
          }
    ...
    if (
      !games.hasOwnProperty(channelObj.name) ||
      (!games[channelObj.name].allMode && games[channelObj.name].player.name !== user["username"])
    ) {
      return;
    }
...
function update(channel) {
  ...
  if (!found) gameObj.snake.popTail();
...
async function gameOver(gameObj, won = false) {
  if (!won) {
    gameObj.sayFunc(
      gameObj.channelObj.name,
      `/me GAME OVER! ${gameObj.player.name} got ${gameObj.points} points and earned ${
        gameObj.points * ushReward
      }USh!`
    );
    ...
    gameObj.sayFunc(
      gameObj.channelObj.name,
      `/me Wow you actually won PogChamp ! ${gameObj.player.name} got ${
        gameObj.points
      } points and earned ${gameObj.points * ushReward}USh plus a ${winningBonus}USh winning bonus!`
    );
    await db.addUserPoints(
      gameObj.player.id,
      gameObj.player.name,
      gameObj.points * ushReward + winningBonus
    );
  }
