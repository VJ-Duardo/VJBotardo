...
async function loadChannel(
  ...
  try {
    channelsObjs[name] = new Channel(
      String(id),
      name,
      prefix,
      booleanCheck(modsCanEdit, true),
      booleanCheck(whileLive, true),
      booleanCheck(gifSpam, true),
      banphraseAPI,
      booleanCheck(allowIfPajbotDown, true)
    );
...
function loadCommand(name, cooldown, minCooldown, devOnly, maxCooldown = 600000, changeable = 1) {
  if (
    [name, cooldown, minCooldown, devOnly].includes(undefined) ||
    commandObjs.hasOwnProperty(name)
  )
    return -1;

  if (isNaN(parseInt(cooldown)) || isNaN(parseInt(minCooldown))) return -1;

  maxCooldown =
    typeof maxCooldown === "undefined" || isNaN(parseInt(maxCooldown))
      ? 600000
      : parseInt(maxCooldown);

  commandObjs[name] = new Command(
    name,
    parseInt(cooldown),
    parseInt(minCooldown),
    maxCooldown,
    booleanCheck(devOnly, false),
    booleanCheck(changeable, true)
  );
  return 1;
}
...
const sayFunc = async function (channel, nMessage) {
  ...
  if (channelsObjs[channel].banphraseAPI !== null && !brailleData.pureBrailleRegex.test(nMessage)) {
    return fetch(`https://${channelsObjs[channel].banphraseAPI}/api/v1/banphrases/test`, {
      ...
      .catch(async () => {
        await client.me(
          channel,
          "The banphrase API cannot be reached at this moment. If you want to allow messages anyway, set the bot option allowIfPajbotDown to true."
        );
      });
  } else {
    await client.privmsg(channel, nMessage);
...
async function getTop(channel, type) {
  ...
  if (topString !== -1) sayFunc(channel, `/me ${topString}`);
}
...
function ping(channel) {
  ...
  client
    ...
    .then(() => {
      sayFunc(
        channel,
        `/me BING! (${new Date().getTime() - pingStart}ms). \
...
function about(channel) {
  sayFunc(
    channel,
    `/me A bot by Duardo1. Command list can be found here: https://gist.github.com/VJ-Duardo/ee90088cb8b8aeec623a6092eaaa38bb Used prefix in this channel: ${channelsObjs[channel].prefix}`
  );
}
...
function commands(channel) {
  sayFunc(
    channel,
    "/me A command list can be found here: https://gist.github.com/VJ-Duardo/ee90088cb8b8aeec623a6092eaaa38bb"
  );
}
...
async function allowanceCheck(channel, user, command, callback, params) {
  ...
  if (!channelObj || !commandObj) return -1;
  ...
  if (!config.devIDs.includes(user["user-id"])) {
    ...
    if (
      typeof commandObj.devOnly !== "undefined" &&
      commandObj.devOnly &&
      !config.devIDs.includes(user["user-id"])
    )
      return -1;

    if (!(await commandObj.getEnabledStatus(channelObj.id))) return -1;

    if (!channelObj.whileLive && !["setcommand", "setbot"].includes(command.toLowerCase())) {
      if (await getLiveStatus(channelObj.id, channel)) return -1;
    }
  }
  ...
  if (now >= channelObj.lastCommandTime[command] + cooldown) {
    ...
    console.log(
      `${channel}, ${command}, ${params.filter(
        (par) => typeof par !== "function" && (typeof par !== "object" || Array.isArray(par))
      )}`
    );
    try {
      callback(...params);
...
async function addCommand(channel, name, cooldown, minCooldown, devOnly, changeable, maxCooldown) {
  ...
  let insertStatus = await db.insertNewCommand(
    name,
    cooldown,
    minCooldown,
    commandObjs[name].maxCooldown,
    devOnly,
    changeable
  );
  if (insertStatus === 1) {
...
function optionCheck(channel, value, options) {
  if (!options.some(isNaN) && options.length === 2 && !isNaN(value)) {
    if (value >= options[0] && value <= options[1]) return true;
  }
...
function modsCanEditCheck(channelObj, user) {
  return (
    (channelObj.modsCanEdit && user["mod"]) ||
...
async function setBot(channel, user, option, value) {
  let channelObj = channelsObjs[channel];
  if (!modsCanEditCheck(channelObj, user)) return -1;
  ...
  switch (option) {
    case "prefix":
      ...
      if (new RegExp(prefixRegex).test(value)) {
        ...
        sayFunc(
          channel,
          `/me Allowed characters: a-zA-Z0-9^!?"'#$%&()[]{}=+*~\\-_,;@<>° Min length: ${channelObj.minPrefix}, Max length: ${channelObj.maxPrefix}`
        );
        return -1;
      }
      break;
    ...
    case "banphraseAPI":
      ...
      if (value !== "null" && (await isPajbotLinkValid(value)) === false) {
        sayFunc(
          channel,
          "/me The url seems to be invalid. If it's correct, make sure the pajbot is running."
        );
        return -1;
      }
      if (value === "null") {
  ...
  if (dbStatus === 1) sayFunc(channel, `/me Changed option ${option} to ${value}`);
  else sayFunc(channel, "/me Something went wrong in the db.");
  return 1;
}
...
async function setCommand(channel, user, command, option, value) {
  ...
  let commandObj = commandObjs[command];
  if (commandObj.devOnly) return -1;
  ...
  if (dbStatus === 1) sayFunc(channel, `/me Changed option ${option} of ${command} to ${value}`);
  else sayFunc(channel, "/me Something went wrong in the db.");
  return 1;
}
...
async function checkCommand(channel, command) {
  ...
  sayFunc(
    channel,
    `/me Settings for command ${command}: cooldown: ${cooldown} sec, enabled: ${enabled}`
  );
}
...
async function suggest(channel, user, content) {
  let status = await fetch("https://api.github.com/repos/VJ-Duardo/VJBotardo/issues", {
    ...
    body: JSON.stringify({
      title: user["username"],
      body: `From #${channel}: ${
        content !== "" ? content : "(no message, most likely channel request)"
      }`,
    }),
  })
    .then((response) => {
...
client.on("USERSTATE", (msg) => {
  if (typeof channelsObjs[msg.channelName] !== "undefined") {
    channelsObjs[msg.channelName].isVipOrMod =
      msg.badges.hasModerator || msg.badges.hasVIP || msg.badges.hasBroadcaster;
  }
});
...
client.on("PRIVMSG", (msg) => {
  ...
  switch (command[0]) {
    ...
    case `${prefix}ping`:
      ...
      break;
    case `${prefix}ush`:
      allowanceCheck(...identParams, showPoints, [
        channel,
        userstate["username"],
        userstate["user-id"],
        command[1],
    ...
    case `${prefix}commands`:
      ...
      break;
    case `${prefix}corona`:
      allowanceCheck(...identParams, corona.corona, [
        channelsObjs[channel],
        sayFunc,
        command.slice(1, command.length).join(" "),
        channelsObjs[channel].gifSpam,
      ]);
      break;
    case `${prefix}ascii`:
      allowanceCheck(...identParams, ascii.printAscii, [
        channelsObjs[channel],
        sayFunc,
        "ascii",
    ...
    case `${prefix}antimirror`:
      ...
      break;
    case `${prefix}ra`:
      allowanceCheck(...identParams, ascii.randomAscii, [
        channelsObjs[channel],
        sayFunc,
        channelsObjs[channel].gifSpam,
        command.slice(1, command.length),
    ...
    case `${prefix}addCommand`:
      allowanceCheck(...identParams, addCommand, [
        channel,
        command[1],
        command[2],
        command[3],
    ...
    case `${prefix}setbot`:
    case `${prefix}setBot`:
      allowanceCheck(channel, userstate, "setBot", setBot, [
        channel,
        userstate,
        command[1],
        command[2],
    ...
    case `${prefix}setCommand`:
      allowanceCheck(channel, userstate, "setCommand", setCommand, [
        channel,
        userstate,
        command[1],
        command[2],
    ...
    case `${prefix}suggest`:
      ...
      break;
    case `${prefix}robneedshiscookiesrightfuckingnow`:
      allowanceCheck(
        ...identParams,
        async (channel) => {
          const url = `http://yerkee.com/api/fortune`;
          let data = (await (await fetch(url)).json()).fortune;
          sayFunc(channel, data.replace(/[\r\n]/gm, ""));
        },
        [channel]
      );
      break;
  ...
  if (channelsObjs[channel].gameRunning) {
    ...
    switch (command[0]) {
      case `${prefix}guess`:
        allowanceCheck(...identParams, guess.guessTheEmote, [
          channelsObjs[channel],
          sayFunc,
          userstate,
          command,
        ...
        break;
      case `${prefix}ttt`:
        allowanceCheck(...identParams, ttt.tictactoe, [
          channelsObjs[channel],
          sayFunc,
          userstate,
          command,
        ...
        break;
      case `${prefix}snake`:
        allowanceCheck(...identParams, snake.playSnake, [
          channelsObjs[channel],
          sayFunc,
          userstate,
          command,
        ...
        break;
      case `${prefix}darts`:
        allowanceCheck(...identParams, darts.playDarts, [
          channelsObjs[channel],
          sayFunc,
          userstate,
          command,
